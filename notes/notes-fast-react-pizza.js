 ...
 * @ Remember our very first project? - 👉 Now the same restaurant (bussiness) needs a simple way of allowing customers to order pizza and get them delivered to their home - 👉 We were hired to build the application front-end
 *
 * How to plan and build a React application:
 * @ From the earlier "Thinking in React" lectures: 1 - Break the desired UI into components 2 - Build a static version (no state UI) 3 - Think about state management + data flow 👇👇 ++ - This works well for small apps with one page and a few features ++ - In real-world apps, we need to adapt this process
 *
 ...
 * @ Step 1: 1 - Very simple application,where users can order one or more pizzas from a menu 2 - Requires no user accounts and no login:users just input their names before using the app 3 - The pizza menu can change, so it should be loaded from an API (DONE✅) 4 - Users can add multiple pizzas to a cart before odrering 5 - Ordering requires just the user's name,phone number and address 6 - If possible,GPS location should be also be provided to make delivery easier 7 - Users can make their orders as "priority" for an additional 20% of the cart price 8 - Orders are made by sending a POST request with the order data (user data + selected pizzas) to the API 9 - Payments are made on delivery, so no payment processing is necessary to the app 10 - Each order will get a unique ID that should be displayed,so the user can later look up their order based on the ID 11 - Users should be able to mark their orders as "priority" order even after it has been placed
 *
 * ! HINT: From these requirements,we can undrestand the features we need to implement
 *
 * @ Step 2 + 3 (Features + Pages) : ++ Feature categories: 1 - User (For the HomePage) 2 - Menu (For the Pizza menu) 3 - Cart (For the Cart) 4 - Order (One for placing a new order and one for looking up an order)
 *
 ...
 * @ - First we create a new variable named errors and then we set a condition for it and set the value of the errors by that condition ++ Example: const errors = {} if(!isValidNumber) errors.phone = "Please enter a correct number";
 *
 * @ - Second we set a new condition which says if the errors object length was more than 0 then return it: ++ Example: if(Object.keys(errors).length > 0) return errors;
 *
 * @ - Third we might use a custom hook named useActionData imported from react-router-dom and then store it into an variable to get the errors: ++ Example: import {useActionData} from 'react-router-dom' const errors = useActionData();
 *
 * useActionData:
 ...
 * useEffect(function(){
 *   fetcher.load("(the route that we want to get data from)");
 * },[fetcher])
 *
 * @format
 */
